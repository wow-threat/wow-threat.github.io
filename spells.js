...
function handler_basic(ev, fight) {
	switch (ev.type) {
	case "damage":
		threatFunctions.sourceThreatenTarget(ev, fight, ev.amount + ev.absorbed);
		break;
	case "heal":
...
function handler_modDamage(multiplier) {
	return (ev, fight) => {
		if (ev.type !== "damage") return;
		threatFunctions.sourceThreatenTarget(ev, fight, ev.amount + ev.absorbed, true, multiplier);
	}
}
...
function handler_modDamagePlusThreat(multiplier, bonus) {
	return (ev, fight) => {
		...
		threatFunctions.sourceThreatenTarget(ev, fight, multiplier * (ev.amount + ev.absorbed) + bonus);
	}
}
...
function handler_threatOnHit(threatValue) {
    return (ev, fight) => {
		if (ev.type !== "damage" || ev.hitType > 6 || ev.hitType === 0) return;
		threatFunctions.sourceThreatenTarget(ev, fight, ev.amount + ev.absorbed + threatValue);
    }
}

function handler_bossDropThreatOnHit(pct) {
	return (ev, fight) => {
		// hitType 0=miss, 7=dodge, 8=parry, ...
		if (ev.type !== "damage" || ev.hitType > 6 || ev.hitType === 0) return;
		let a = fight.eventToUnit(ev, "source");
		let b = fight.eventToUnit(ev, "target");
		if (!a || !b) return;
